...
<div class="content-wrap"> 
	...
	<%-- 타이틀 --%>
   	<div class="board-address">
		<span class="board-copy">글 주소 복사 <a href="#">www.asdawdas.com</a></span>
	...
	<%-- 추천,비추천,신고 --%>
   	<div class="board-detail-button">
   		<c:choose>
   			<c:when test="${upChk==1 }">
   				<a href="#" class="good">추천 : <span>${upCount}</span></a>
   			</c:when>
   			...
   			<c:otherwise>
   				<a href="updown?boardNum=${param.boardNum}&updown=1&userId=${sessionScope.user_info.getUserId()}" class="good">추천 : <span>${upCount }</span></a>
   		   		...
   		   		<c:choose>
   			<c:when test="${downChk==1}">
   				<a href="#" class="bad">비추천 : <span>${downCount}</span></a>
   			</c:when>
   			<c:when test="${sessionScope.user_info==null }">
   				<a href="#" class="bad">비추천 : <span>${downCount}</span></a>
   			</c:when>
